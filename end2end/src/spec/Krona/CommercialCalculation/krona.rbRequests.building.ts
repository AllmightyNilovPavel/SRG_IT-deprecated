...
import { baikalCalc, baikalFavorites, baikalMap } from "pages/baikal";

describe.skip(`КРОНА. СМОК. КН. Магазин.`, function () {
  ...
  it(`Переход на страницу тестового расчёта.`, function () {
    browser.url(TestDataKrona.rbRequest.building);
  ...
  it(`Открыть расчётник.`, function () {
    ...
    expect(baikalCalc.$calcTableData_mainObjectLink.isDisplayed()).to.be.true;
