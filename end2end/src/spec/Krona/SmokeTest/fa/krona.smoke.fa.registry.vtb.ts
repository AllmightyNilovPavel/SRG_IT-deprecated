...
describe.skip(`КРОНА. СМОК. VTB. Реестр Федерального Оценщика. Старый.`, function () {
  ...
  it(`Проверка реестра`, function () {
    allureReporter.generateReport();
    let result_check;
    result_check = kronaResultTable.resultTableGetData(KronaDataType.FA_PUBLIC);
    expect(result_check).to.be.not.null;
    makeScreenshot("KRONA_SMOKE_FA_VTB_OLD");
