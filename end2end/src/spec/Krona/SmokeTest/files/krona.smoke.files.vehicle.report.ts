...
describe(`КРОНА. СМОК. SRG. Файлы. БЗО-ТС. Отчёт.`, function () {
  ...
  it(`Скачать отчёт`, function () {
    allureReporter.generateReport();
    kronaResultTable.resultTable_download(KronaFileType.vehicle_report).scrollIntoView();
    kronaResultTable.resultTable_download(KronaFileType.vehicle_report).click();
    browser.pause(1500);
    expect(kronaResultTable.resultTable_download(KronaFileType.vehicle_report).isClickable()).to.be
      .true;
  ...
  it(`Скриншот скачанных файлов`, function () {
    ...
    browser.pause(1000);
    makeScreenshot("SMOKE_Файлы_БЗО-ТС-отчёт");
  });
});
