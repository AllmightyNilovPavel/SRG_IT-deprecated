...
describe(`КРОНА. СМОК. SRG. Файлы. Реестр отчётов. Файл отчёта со штампом ЭЦП.`, function () {
  ...
  it(`Скачать отчёт со штампом ЭЦП`, function () {
    allureReporter.generateReport();
    let target = kronaResultTable.resultTable_download(KronaFileType.digital_sign_report);
    ...
    makeScreenshot("Krona_smoke_РеестрОтчётов-отчётСоШтампомЭцп");
  });
});
