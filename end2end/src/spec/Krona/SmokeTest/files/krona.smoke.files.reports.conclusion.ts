...
describe(`КРОНА. СМОК. SRG. Файлы. Реестр отчётов. Файл заключения`, function () {
  ...
  it(`Скачать заключение`, function () {
    allureReporter.generateReport();
    let target = kronaResultTable.resultTable_download(KronaFileType.conclusion);
    target.scrollIntoView({ block: "center", inline: "center" });
    target.click();

    browser.pause(1500);
    expect(kronaErrorPage.$errorBody.isDisplayed()).to.be.false;
    makeScreenshot("Krona_smoke_РеестрОтчётов-заключение");
  });
});
