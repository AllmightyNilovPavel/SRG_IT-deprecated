...
describe(`КРОНА. SRG. Реестр объектов. ТАУНХАУС. Новый запрос на оценку.`, function () {
  ...
  it(`Открыть страницу создания нового ЗАПРОСА`, function () {
    ...
    expect(requestsCreateTownhouse.$map.isExisting()).to.be.true;
  });
  it(`Заполнить форму`, function () {
    allureReporter.generateReport();
    expect(browser.getUrl()).to.include(requestsCreateTownhouse.path);
    requestsCreateTownhouse.FullfillTownhouseRequest(TestDataKrona.Request.Flat.IDEAL);
    this.retries(3);
  });
