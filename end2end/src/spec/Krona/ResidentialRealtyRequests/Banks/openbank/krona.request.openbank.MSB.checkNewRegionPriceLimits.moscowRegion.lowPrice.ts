...
describe(`КРОНА. ОТКРЫТИЕ. МСБ. Новые условия проверки дорогих объектов. Московская область. Ниже лимита.`, function () {
  let requestId: string = "";
  const TestData = KronaTestDataOpenbankRequestsOverprice.FLAT.MOSCOW_REGION.LOW_PRICE;
  ...
  it(`Проверяем статус`, function () {
    allureReporter.generateReport({
      description: `Так как цена запроса МЕНЬШЕ 50 млн и эксперт оценил МЕНЬШЕ 50 млн - статус запроса после верификации должен быть "${KronaResidentialReportStatusesTranslated.ACCEPTED_EXPERT_VALUATION}"`,
    });
