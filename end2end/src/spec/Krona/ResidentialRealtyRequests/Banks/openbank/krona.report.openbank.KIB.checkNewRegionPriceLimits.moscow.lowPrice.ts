...
describe(`КРОНА. ОТЧЁТ. ОТКРЫТИЕ. КИБ. Новые условия проверки дорогих объектов. Москва. Ниже лимита.`, function () {
  ...
  it(`Заполнить форму`, function () {
    allureReporter.generateReport({
      ...
      ${JSON.stringify(KronaTestDataOpenbankReportsOverprice.FLAT.MOSCOW.LOW_PRICE, void 2, 2)}`,
    });
    browser.refresh();
    reportsCreateFlat.waitForLoad();

    reportsCreateFlat.FullfillFlatRequest(
      KronaTestDataOpenbankReportsOverprice.FLAT.MOSCOW.LOW_PRICE
    );
      ...
      additionalArguments: [
        {
          ...
          argValue: `${KronaTestDataOpenbankReportsOverprice.FLAT.MOSCOW.LOW_PRICE.custom1}`,
        },
      ],
  ...
  //   kronaFiltersBox.inputCustomField(TestData.FLAT.MOSCOW.LOW_PRICE.custom1, 1);
  ...
  it(`Оцениваем запрос`, function () {
    ...
    for (let i = 1; i < 4; i++) {
      kronaExpertRequestCard
        ...
        .setValue(KronaTestDataOpenbankReportsOverprice.FLAT.MOSCOW.LOW_PRICE.price - 9000000);
      kronaExpertRequestCard
        .comparables(i, KronaExpertResponseComparableFieldNames.TOTAL_SPACE)
        .setValue(KronaTestDataOpenbankReportsOverprice.FLAT.MOSCOW.LOW_PRICE.total_space);
      kronaExpertRequestCard
        .comparables(i, KronaExpertResponseComparableFieldNames.REPAIRS_TYPE)
        .selectByIndex(
          KronaTestDataOpenbankReportsOverprice.FLAT.MOSCOW.LOW_PRICE.flatRepairs_index
        );
    }
    kronaExpertRequestCard.checkDoubles();
    kronaExpertRequestCard.checkHouseHistory();
