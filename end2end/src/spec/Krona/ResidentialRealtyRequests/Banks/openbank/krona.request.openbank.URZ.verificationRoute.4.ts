...
describe(`КРОНА. ОТКРЫТИЕ. Маршрутка. УРЗ. Кейс №4.`, function () {
  ...
  it(`Логин в Крону под СОТРУДНИКОМ банка`, function () {
    ...
    kronaLoginPage.login(
      TestDataKrona.Users.login.openbank.autotest_URZ,
      TestDataKrona.Users.password.openbank.autotest_URZ
    );
  ...
  it(`Принимаем ОТРИЦАТЕЛЬНОЕ решение`, function () {
    allureReporter.generateReport({ description: `` });
    let uziPriceReferrence: number = 6000000;
    ...
    kronaDpaUziDecision.makeUziDecision(
      ...
      uziPriceReferrence,
      "Кейс №4. Возврат на доработку в SRG"
    );
    kronaRequestCard.goToRequestCard(requestId);
    let requestStatus: string = kronaRequestCard.$info_requestStatus.getText();
    expect(requestStatus).to.include(
  ...
  it(`Оцениваем запрос на стоимость ВЕРИФИКАЦИИ`, function () {
    allureReporter.generateReport({
      description: `Делаем повторную верификацию после 
    ...
    if (
      !kronaExpertRequestCard
        .comparables(1, KronaExpertResponseComparableFieldNames.PRICE)
        .isEnabled()
    ) {
      browser.pause(60000);
      browser.refresh();
    ...
    for (let i = 1; i < 4; i++) {
      ...
      kronaExpertRequestCard
        .comparables(i, KronaExpertResponseComparableFieldNames.REPAIRS_TYPE)
        .selectByIndex(TestData.flatRepairs_index + 1);
    }
    kronaExpertRequestCard.saveResponseDraft();
    kronaExpertRequestCard.commitJuniorAnalystResponse();
    kronaExpertRequestCard.confirmationBySenior();
