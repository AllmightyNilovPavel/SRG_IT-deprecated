...
describe(`КРОНА. ОТКРЫТИЕ. МСБ. Новые условия проверки дорогих объектов. Ленинградская область. Больше лимита.`, function () {
  let requestId: string = "";
  const TestData = KronaTestDataOpenbankRequestsOverprice.FLAT.PITER_REGION.HIGH_PRICE;
  ...
  it(`Логин в Крону`, function () {
    ...
    kronaLoginPage.login(
      TestDataKrona.Users.login.openbank.autotest_MSB,
      TestDataKrona.Users.password.openbank.autotest_MSB
    );
  ...
  it(`Переход на страницу создания нового ЗАПРОСА`, function () {
    ...
    requestsCreateTownhouse.waitForLoad();
  });
  it(`Заполнить форму`, function () {
    ...
    requestsCreateTownhouse.FullfillTownhouseRequest(TestData);
  });
  it(`Отправить запрос и ждать ответ`, function () {
    ...
    requestsCreateTownhouse.$button_buttonSend.click();
    valuationCheck = requestsCreateTownhouse.waitForResult(ValuationResultData.STATUS);
  ...
  it(`Оцениваем запрос`, function () {
    ...
    kronaExpertRequestCard.fillDataFromAnotherRequest("285224");
