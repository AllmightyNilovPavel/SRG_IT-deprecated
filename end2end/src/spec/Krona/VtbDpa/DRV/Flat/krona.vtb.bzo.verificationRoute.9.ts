...
Недорогой объект. Отклонение > 30%. Верификация СТЗ. Эталон СТЗ.
Доработка в ОК. Отклонение > 30%. Верификация СТЗ.`;
...
describe(testName, function () {
  ...
  it(`КРОНА. Заполнение заказа ДРВ.`, function () {
    ...
    setOrderData.inputObjectAddress(testDataGenerated.addressWithFlatNumber);
  ...
  it(`Оценка эксперта SRG`, function () {
    ...
    for (let i = 1; i < 4; i++) {
      ...
      kronaExpertRequestCard
        ...
        .setValue(toNumber(reportPrice) + 3000000);
      kronaExpertRequestCard
  ...
  it(`Проверка статуса отчёта в заказе`, function () {
    allureReporter.generateReport({
      ...
      с отклонением больше 10% но меньше 30% от цены отчёта то:
      1) Отчёт получает статус "Готово. Изменена стоимость" (или Скорректировно)
      2) В столбец "Замечания к отчёту" проставляется стандартный текст
      3) Заказ получает статус "На доработке"`,
    });
    kronaNavigationBar.logout();
    kronaLoginPage.waitForLoad();
    kronaLoginPage.login(
    ...
    expect(
      kronaOrderDpaResidentialOrderCard.VERIFICATION_DATA_TAB.$verificationReportStatus.getText(),
      errorMsg
    ).to.be.equal("Готово. Изменена Стоимость");
    makeScreenshot("Проверка статуса отчёта по заказу.");
  });
  it(`Проверка статуса заказа.`, function () {
    allureReporter.generateReport({
      description: `После успешной проверки статуса
        ...
        Статус отчёта должен быть равен "На доработке".`,
    });
    ...
    expect(orderStatus.getText(), errorMsg).to.be.equal("На доработке");
    makeScreenshot(`Проверка статуса заказа`);
  });
  it(`БО. Изменение данных по отчёту`, function () {
    ...
    residentialReportPage.Navigation.navigateTo(BaReportNavigationPanels.ANALOGS);
    for (let i = 1; i < 4; i++) {
      residentialReportPage.AnalogData.setAnalogTextData(i, "31000000", BaAnalogDataField.PRICE);
    }
    residentialReportPage.BasicActions.saveReport();
    residentialReportPage.BasicActions.signReport(
      BaResidentialSignType.PASSWORD,
