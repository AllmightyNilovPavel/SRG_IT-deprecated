...
import {
  ...
  reportsCreateNewApartment,
  kronaResultTable,
  reportsCreateApartment,
  kronaFiltersBox,
} from "pages/Krona";
import { TestDataKrona } from "options/testData/krona";
import { makeScreenshot } from "modules";

describe(`КРОНА. SRG. Реестр отчётов. АПАРТАМЕНТЫ НОВОСТРОЙКА. Новый запрос на верификацию.`, function () {
  ...
  it(`Переход на страницу создания нового Отчёта.`, function () {
    ...
    reportsCreateNewApartment.waitForLoad();
    // expect(reportsCreateNewApartment.$map.isExisting()).to.be.true;
  });
  it(`Заполнить форму`, function () {
    allureReporter.generateReport();
    expect(browser.getUrl()).to.include(reportsCreateNewApartment.path);
    reportsCreateNewApartment.FullfillNewApartmentRequest(TestDataKrona.Request.NewApartment);
    reportsCreateNewApartment.FullfillReportData(TestDataKrona.Report.Flat);
    generatedReportNumber = reportsCreateApartment.getReportNumber();

    this.retries(3);
  });
  it(`Отправка запроса и ожидание перехода в реестр отчётов.`, function () {
    allureReporter.generateReport();
    let buttonSend = reportsCreateNewApartment.$button_buttonSend;
