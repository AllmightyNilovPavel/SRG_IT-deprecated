...
describe(`КРОНА. SRG. Реестр отчётов. АПАРТАМЕНТЫ. Новый запрос на верификацию.`, function () {
  ...
  it(`Переход на страницу создания нового Отчёта.`, function () {
    ...
    reportsCreateApartment.waitForLoad();
  });
  it(`Заполнить форму`, function () {
    allureReporter.generateReport();
    expect(browser.getUrl()).to.include(reportsCreateApartment.path);
    reportsCreateApartment.FullfillApartmentRequest(TestDataKrona.Request.Flat.IDEAL);
    reportsCreateApartment.FullfillReportData(TestDataKrona.Report.Flat);
  ...
  it(`Отправка запроса и ожидание перехода в реестр отчётов.`, function () {
    allureReporter.generateReport();
    let buttonSend = reportsCreateApartment.$button_buttonSend;
