...
describe(`КРОНА. Коммерческая Недвижимость. ВТБ. Файлы. Заключение к отчёту. Статус - Готово.`, function () {
  ...
  it(`Поиск отчёта`, function () {
    allureReporter.generateReport({
      description: `Сортируем реестр по "Дате ОТ":${options.Date.startOfCurrentYear}  и по Статусу: ${KronaCommercialVtbVerificationStatus.READY}`,
    });
    kronaFiltersBox.$inputDateBegin.setValue(options.Date.startOfCurrentYear);
    kronaFiltersBox.selectVerificationStatus(KronaCommercialVtbVerificationStatus.READY);
