...
describe(`КРОНА. Коммерческая Недвижимость. ОТКРЫТИЕ. Файлы. Заключение к отчёту. Статус - "Скорректировано'.`, function () {
  ...
  it(`Поиск отчёта`, function () {
    allureReporter.generateReport({
      description: `Сортируем реестр по Статусу: ${KronaCommercialOpenbankVerificationStatus.READY_PRICE_CHANGED}`,
    });
    kronaFiltersBox.selectVerificationStatus(
      KronaCommercialOpenbankVerificationStatus.READY_PRICE_CHANGED
    );
