...
import {
  baikalDemoBeta,
  baikalLeftPanelBetaStatistics,
  baikalMapBeta,
} from "pages/baikal/beta/index";
import { BaikalEnumDemoModeHousingType, BaikalEnumLeftPanelButtons } from "pages/baikal/enums";
import { expect } from "chai";
import { makeScreenshot } from "modules/screenshotsMaker";

let testName: string = `Байкал. Открытие раздела "Статистика" из левой панели Байкала`;
...
describe(testName, function () {
  ...
  it(`Проверить, что при нажатии на раздел "Статистика" из левой панели Байкала - раздел "Статистика" откроется`, function () {
    allureReporter.generateReport();
    expect(
      baikalLeftPanelBetaStatistics.$stats_panel_root.isDisplayed(),
      `При открытии раздела "Карта" раздел "Статистика" из левой панели НЕ должен быть открытым.`
    ).to.be.false;
    makeScreenshot(`1`);
    baikalLeftPanelBetaStatistics.goToSectionFromLeftMenu(BaikalEnumLeftPanelButtons.STATISTICS);
    baikalLeftPanelBetaStatistics.waitForLoad();
    expect(
      baikalLeftPanelBetaStatistics.$stats_panel_root.isDisplayed(),
      `После клика по разделу "Статистика" из левой панели Байкала раздел "Статистика" должен открыться, но
      этого не произошло`
    ).to.be.true;
    makeScreenshot(`2`);
  });
