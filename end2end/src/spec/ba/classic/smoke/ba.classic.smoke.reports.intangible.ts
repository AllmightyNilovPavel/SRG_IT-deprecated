...
describe(`BA. CLASSIC. SMOKE. Реестр отчётов. Нематериальные активы.`, function () {
  ...
  it(`Перейти на вкладку "Нематериальные активы"`, function () {
    allureReporter.generateReport();
    baClassicNavMenu.navigateTo(BaNavigationButtons.INTANGIBLE);
    baClassicReportsTable.waitForLoad();
    expect(browser.getUrl()).to.include("/intangible");
  });
