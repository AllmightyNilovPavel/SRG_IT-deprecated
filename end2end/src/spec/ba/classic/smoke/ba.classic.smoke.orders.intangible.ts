...
import { baLoginPage, baMainPage, intangibleOrdersList } from "pages/ba/classic";
import { TestDataBa } from "options/testData/ba";
import { makeScreenshot } from "modules";

describe(`БО. СМОК. Реестр заказов. Нематериальные активы.`, function () {
  ...
  it(`Перейти в реестр заказов Нематериальных активов`, function () {
    allureReporter.generateReport();
    intangibleOrdersList.open();
    intangibleOrdersList.waitForLoad();
    expect(browser.getUrl()).to.include(intangibleOrdersList.path);
  });
  it(`Проверка наличия данных в таблице заказов`, function () {
    allureReporter.generateReport();
    let temp = intangibleOrdersList.$$table_content;
