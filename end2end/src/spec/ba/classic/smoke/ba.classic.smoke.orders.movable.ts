...
import { baLoginPage, baMainPage, movableOrdersList } from "pages/ba/classic";
import { TestDataBa } from "options/testData/ba";
import { makeScreenshot } from "modules";

describe(`БО. СМОК. Реестр заказов. Движимое имущество.`, function () {
  ...
  it(`Перейти в реестр заказов Движимое имущество`, function () {
    allureReporter.generateReport();
    movableOrdersList.open();
    movableOrdersList.waitForLoad();
    expect(browser.getUrl()).to.include(movableOrdersList.path);
  });
  it(`Проверка наличия данных в таблице заказов`, function () {
    allureReporter.generateReport();
    let temp = movableOrdersList.$$table_content;
