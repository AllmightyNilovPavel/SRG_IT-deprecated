...
describe(`БО. СМОК. Реестр заказов. Недвижимость ДПА. Карточка заказа.`, function () {
  ...
  it(`Переход в карточку первого заказа в таблице`, function () {
    allureReporter.generateReport();
    let target = baDpaOrdersList.$table_firstElement;
  ...
  it(`Проверка загрузки карточки заказа`, function () {
    ...
    browser.waitUntil(
      () =>
        browser.getUrl().match(baDpaValuationOrdersCard.path) !== null &&
