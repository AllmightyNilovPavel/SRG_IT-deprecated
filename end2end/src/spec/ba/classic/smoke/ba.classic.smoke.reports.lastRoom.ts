...
describe(`BA. CLASSIC. SMOKE. Реестр отчётов. Последняя комната.`, function () {
  ...
  it(`Перейти на вкладку "Последняя комната"`, function () {
    allureReporter.generateReport();
    baClassicNavMenu.navigateTo(BaNavigationButtons.LAST_ROOM);
    baClassicReportsTable.waitForLoad();
    expect(browser.getUrl()).to.include("/last_room");
  });
