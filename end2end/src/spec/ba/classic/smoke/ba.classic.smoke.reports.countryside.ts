...
describe(`БО. СМОК. Реестр отчётов. Загородная недвижимость.`, function () {
  ...
  it(`Перейти на вкладку "Загородная недвижимость"`, function () {
    allureReporter.generateReport();
    baClassicNavMenu.navigateTo(BaNavigationButtons.COUNTRYSIDE);
    baClassicReportsTable.waitForLoad();
    expect(browser.getUrl()).to.include("/countryside");
  });
