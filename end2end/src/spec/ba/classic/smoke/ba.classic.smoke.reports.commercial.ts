...
describe(`БО. СМОК. Реестр отчётов. Коммерческая недвижимость.`, function () {
  ...
  it(`Перейти на вкладку "Коммерческая недвижимость"`, function () {
    allureReporter.generateReport();
    baClassicNavMenu.navigateTo(BaNavigationButtons.COMMERCIAL);
    baClassicReportsTable.waitForLoad();
    expect(browser.getUrl()).to.include("/commercial");
  });
