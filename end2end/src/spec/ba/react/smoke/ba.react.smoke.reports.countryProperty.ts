...
describe(`БО. РЕАКТ. СМОК. Реестр отчётов. Жилые Дома.`, function () {
  ...
  it(`Перейти на вкладку "Жилые дома"`, function () {
    allureReporter.generateReport();
    reactNavMenu.navidateTo_secondary(react_NavMenuButtonsSecondary.COUNTRY_PROPERTY);
    reactReportsTable.waitForLoad();
    expect(browser.getUrl()).to.include("/reports/country_property");
    makeScreenshot("REACT_SMOKE_BA_ЖилыеДома");
  });
});
