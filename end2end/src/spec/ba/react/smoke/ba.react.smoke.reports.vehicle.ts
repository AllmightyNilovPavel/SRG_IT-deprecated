...
describe(`БО. РЕАКТ. СМОК. Реестр отчётов. Авто.`, function () {
  ...
  it(`Перейти на вкладку "Автомобили"`, function () {
    allureReporter.generateReport();
    reactNavMenu.navidateTo_secondary(react_NavMenuButtonsSecondary.VEHICLE);
    reactReportsTable.waitForLoad();
    expect(browser.getUrl()).to.include("/reports/auto");
    makeScreenshot("REACT_SMOKE_BA_Автомобили");
  });
});
