...
describe(`БО. РЕАКТ. СМОК. Реестр отчётов. Загородная Недвижимость.`, function () {
  ...
  it(`Перейти на вкладку "Загородная недвижимость"`, function () {
    allureReporter.generateReport();
    reactNavMenu.navidateTo_secondary(react_NavMenuButtonsSecondary.COUNTRYSIDE);
    reactReportsTable.waitForLoad();
    expect(browser.getUrl()).to.include("/reports/countryside");
    makeScreenshot("REACT_SMOKE_BA_Загородка");
  });
});
