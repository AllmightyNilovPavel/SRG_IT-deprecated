...
describe(`FA. ПСБ. End2end. Военная ипотека. Рефинансирования`, function () {
  ...
  it(`Выбор типа Кредитования`, function () {
    ...
    let errorMsgMilitaryRefin:string = 'Кнопка Военная ипотека недоступна или не кликабельна'
    let errorMsgMilitaryRefinCredit :string = 'Кнопка Рефин для военной ипотеки недоступна или не кликабельна'
    ...
    psbOcenka.$MilitaryMortgage.waitForClickable();
     expect(psbOcenka.$MilitaryMortgage.isClickable(),errorMsgMilitaryRefin).to.be.true;
     psbOcenka.$MilitaryMortgage.click();
    psbOcenka.$MilitaryRefin.waitForClickable();
     expect(psbOcenka.$MilitaryRefin.isClickable(),errorMsgMilitaryRefinCredit).to.be.true;
     makeScreenshot("выбор_типа_кредитования");
    psbOcenka.$MilitaryRefin.click();
  });
