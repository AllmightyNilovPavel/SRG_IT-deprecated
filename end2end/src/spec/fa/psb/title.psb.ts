...
describe(`FA. ПСБ. End2end. Титул.`, function () {
  ...
  it(`Зайти в кабинет`, function () {
    ...
    it(`Выбор типа Кредитования`, function () {
      ...
      let errorTittleСredit :string = 'Кнопка Для оформления новостройки в собственность/регистрации закладной недоступна или не кликабельна'
      ...
      psbOcenka.$ButtonTittleСredit.waitForClickable();
       expect(psbOcenka.$ButtonTittleСredit.isClickable(),errorTittleСredit).to.be.true;
       makeScreenshot("выбор_типа_кредитования");
      psbOcenka.$ButtonTittleСredit.click();
    });
    ...
    it(`Выбор типа объекта`, function () {
      allureReporter.generateReport();
      let errorMsgLabelApartments :string = 'Чек бокс выбора Типа объекта не доступна'
  
      psbOcenka.$LabelApartments.waitForClickable();
       expect(psbOcenka.$LabelApartments.isClickable(),errorMsgLabelApartments).to.be.true;
      psbOcenka.$LabelApartments.click();
    });
